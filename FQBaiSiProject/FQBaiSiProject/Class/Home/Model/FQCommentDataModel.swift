...
class FQCommentDataModel: NSObject {
    var canLoadMore = true
    var hotComments = [FQCommentModel]()
    var newComments = [FQCommentModel]()
    var page: Int = 0
        ...
        Alamofire.request(url!, method: .get, parameters: parameter).responseJSON{ (response) in
            ...
            case let .success(value):
                ...
                if let hotData = data["hot"] as? [Any] {
                    for dic in hotData {
                        self.hotComments.append(FQCommentModel(dict: dic as! [String : Any]))
                    }
                }
                
                if let newData = data["data"] as? [Any] {
                    for dic in newData {
                        self.newComments.append(FQCommentModel(dict: dic as! [String : Any]))
                    }
                }
                //百思不得其解返回的total类型有string, 也有Int类型, 还有就是这里加载更多总是比total的少, 不知道原因在哪里
                var allCount = 0
