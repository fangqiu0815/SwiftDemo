...
class FQPublishViewController: UIViewController {
    ...
    var buttons: [FQVeticalSqureBtn] = [FQVeticalSqureBtn]()
...
extension FQPublishViewController
{
    fileprivate func setupUI() {
        let chooseItemArray = [("publish-video", "发视频"),
                               ("publish-picture", "发图片"),
                               ("publish-text", "发段子"),
                               ("publish-audio", "发声音"),
                               ("publish-review", "审帖"),
        ...
        for i in 0...5 {
            let button = FQVeticalSqureBtn()
            button.setTitle(chooseItemArray[i].1, for: .normal)
            button.setTitleColor(#colorLiteral(red: 0, green: 0, blue: 0, alpha: 1), for: .normal)
            button.titleLabel?.font = UIFont.systemFont(ofSize: 13)
            button.setImage(UIImage(named: chooseItemArray[i].0), for: .normal)
                ...
                if i == self.buttons.count {
                    DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.1, execute: {
                        self.dismiss(animated: false, completion: {
                            let vcTag = FQPublishTextViewController()
                            // 这里不能使用self来弹出其他控制器, 因为self执行了dismiss操作
                            let root = UIApplication.shared.keyWindow?.rootViewController
                            let vc = FQNaviViewController(rootViewController:vcTag)
